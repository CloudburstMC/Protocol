...
public class LevelSoundEventPacket_v291 extends LevelSoundEventPacket {
    ...
    public void encode(ByteBuf buffer) {
        buffer.writeByte(sounds.get(sound));
        BedrockUtils.writeVector3f(buffer, position);
        VarInts.writeInt(buffer, extraData);
        VarInts.writeInt(buffer, pitch);
    ...
    public void decode(ByteBuf buffer) {
        sound = sounds.get(buffer.readUnsignedByte());
        position = BedrockUtils.readVector3f(buffer);
        extraData = VarInts.readInt(buffer);
        pitch = VarInts.readInt(buffer);
