...
public class AvailableCommandsSerializer_v363 implements PacketSerializer<AvailableCommandsPacket> {
    private static final InternalLogger log = InternalLoggerFactory.getInstance(AvailableCommandsSerializer_v363.class);
    ...
    public void deserialize(ByteBuf buffer, AvailableCommandsPacket packet) {
        ...
        for (CommandData.Builder command : commands) {
            ...
            List<CommandData.Flag> flags = new ObjectArrayList<>();

            for (int i = 0; i < 8; i++) {
                if (((flagsByte >>> i) & 0x1) != 0) {
            ...
            for (int i = 0; i < overloadBuilders.length; i++) {
                ...
                for (int i2 = 0; i2 < overloadBuilders[i].length; i2++) {
                    ...
                    List<CommandParamData.Option> options = new ObjectArrayList<>();
